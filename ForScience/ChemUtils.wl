...
Block[{Notation`AutoLoadNotationPalette=False},
  BeginPackage["ForScience`ChemUtils`",{"Notation`","ForScience`Util`","ForScience`Usage`"}]
]
...
AdjacencyToBonds[mat_]:=DeleteDuplicates[Bond[##][mat[[##]]]&@@@Position[Normal@mat,_?Positive,{2}]]
...
DrawBond[{s1_,s2_},1,_]:=iDrawBond[s1,s2,0,15]
DrawBond[{s1:{p1_,_},s2:{p2_,_}},t_,{nb1_,nb2_}]:=Let[
  {
    n1=iGetBondNormal[{p1,p2},nb1],
    n2=iGetBondNormal[{p2,p1},nb2],
    offset=Normalize[(n1+If[n1.n2>=0,1,-1]n2)\[Cross](p1-p2)]
  },
  If[t==2,
    iDrawBond[s1,s2,#,8]&/@{14offset,-14offset},
    iDrawBond[s1,s2,#,6]&/@{21offset,0,-21offset}
  ]
]

iDrawBond[{p1_,c1_},{p2_,c2_},offset_,r_]:=Let[
  {
    sp1=p1+offset,
    sp2=p2+offset,
    mid=Mean@{sp1,sp2}
  },
  {{c1,Cylinder[{sp1,mid},r]},{c2,Cylinder[{mid,sp2},r]}}
]
