...
Details[FormatUsage]={
  ...
  TableForm@{
    ...
    {BoxData@StyleBox["{*\!\(\*
StyleBox[\"str\", \"TI\"]\)*}","CodeFont"],"Treats ```str``` as a single token, e.g. for subscripting"},
    {BoxData@StyleBox["[*\!\(\*
StyleBox[\"str\", \"TI\"]\)*]","CodeFont"],"Applied [*FormatUsageCase*] to ```str``` and tags it for hyperlinking (for documentation pages)"}
  },
